...
def get_residual_block(train,
  ...
  with tf.variable_scope("%sresidual%02d_%s" % (prefix, idx, suffix),
    ...
    with tf.variable_scope("conv1"):
      ...
      conv_plus_bias = common.get_conv_plus_bias_block(
          train,
          input_,
          kernel_size,
          n_channels,
      ...
      if n_in_channels != n_out_channels or subsample != 1:
        with tf.variable_scope("shortcut"):
          ...
          if n_in_channels != n_out_channels:
            weight_init = tf.learn.xavier_initializer(n_in_channels,
                                                      n_out_channels,
                                                      uniform=uniform)
            kernel = common.variable_with_weight_decay(
                'weights',
                shape=[1, 1, n_channels, n_filters],
                weight_init=weight_init,
                weight_decay=weight_decay,
                train=train)
            shortcut = tf.nn.conv2d(subsampled,
                                    kernel,
                                    [1, 1, 1, 1],
                                    padding='SAME')
          else:
...
def get_cnn_common(train, images, labels, n_classes, batch_size, img_channels,
                   img_side, initial_n_filters, initial_kernel_size,
                   initial_subsample, residual_blks_per_subsample,
                   min_map_side_for_conv, uniform, weight_decay, dropout_prob,
                   enable_global_avg_pool, activation, reuse):
  ...
  while True:
    for i in xrange(1, residual_blks_per_subsample + 1):
      last_output = get_residual_block(train,
                                       reuse,
                                       activation,
                                       count,
                                       last_output,
  ...
  if dropout_prob is not None:
    ...
    if not enable_global_avg_pool:
      common.activation_image_summary(tf.reshape(
          reshape, [batch_size, current_map_side, current_map_side,
                    old_n_fileters]), reshape.op.name, 3, train)
  with tf.variable_scope('logits', reuse=reuse):
    weight_init = tf.learn.xavier_initializer(current_n_filters,
                                              n_classes,
                                              uniform=uniform)
    fc_weights = common.variable_with_weight_decay(
        'fc_weights',
        shape=[current_n_filters, n_classes],
        weight_init=weight_init,
        weight_decay=weight_decay,
        train=train)
  ...
  return logits, common.cross_entropy_weight_decay_losses(
      logits, labels) if train else None


def get_cnn(train, images, labels, initial_n_filters, initial_kernel_size,
            initial_subsample, residual_blks_per_subsample,
            min_map_side_for_conv, uniform, weight_decay, dropout_prob,
            enable_global_avg_pool, activation, reuse):
  batch_size, img_side, img_side, img_channels = images.get_shape().as_list()
  batch_size, n_classes = labels.get_shape().as_list()
  return get_cnn_common(
      train, images, labels, n_classes, batch_size, img_channels, img_side,
...
class ResNet:
  ...
  def __init__(self,
               ...
               initial_subsample=1,
               residual_blks_per_subsample=2,
               min_map_side_for_conv=6,
               uniform=True,
               weight_decay=None,
               dropout_prob=None,
