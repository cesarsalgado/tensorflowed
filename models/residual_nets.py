...
def get_residual_block(train, activation, idx, input_, kernel_size, n_in_channels, n_out_channels, subsample=1, uniform=True, weight_decay=None, residual=True, suffix=''):
...
def get_cnn_common(train, images, labels, n_classes, batch_size, img_channels, img_side, initial_n_filters,
                   ...
                   dropout_prob, enable_global_avg_pool, activation):
    ...
    while True:
        for i in xrange(1, residual_blks_per_subsample+1):
            last_output = get_residual_block(train, activation, count, last_output, 3, current_n_filters, current_n_filters, uniform=uniform, weight_decay=weight_decay)
            count += 1
        if current_map_side/2 < min_map_side_for_conv:
            break
        next_n_filters = 2*current_n_filters
        last_output = get_residual_block(train, activation, count, last_output, 3, current_n_filters, next_n_filters, subsample=2, uniform=uniform, weight_decay=weight_decay)
...
def get_cnn(train, images, labels, initial_n_filters, initial_kernel_size, initial_subsample,
    ...
    return get_cnn_common(train, images, labels, n_classes, batch_size, img_channels, img_side, initial_n_filters,
                          ...
                          uniform, weight_decay, dropout_prob, enable_global_avg_pool, activation)
