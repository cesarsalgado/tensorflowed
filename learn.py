  ...
  with tf.name_scope('input'):
    with tf.device('/cpu:0'):
      ...
      y_init = tf.placeholder(tf.float32,
                              shape=(dataset.y.shape[0], dataset.n_classes))
...
def run_for_one_epoch(epoch,
                      ...
                      y_var,
                      logits,
                      console_print_step=10,
                      train_op=None,
                      losses=None,
                      learning_rate=None,
                      summary_op=None,
                      summary_writer=None,
                      summary_batch_step=None):
  batch_iter = OneEpochBatchIterator(rand_state, batch_size, dataset,
                                     train_op is not None)
  n_correct = 0
  count = 0
  while batch_iter.has_next():
    if train_op is None:
      x, y, n_used = batch_iter.next()
      feed_dict = {x_var: x, y_var: y}
      y_pred = logits.eval(feed_dict=feed_dict)
      phase = 'Test'
    else:
      batch_iter.next()
      if None in [logits, losses, learning_rate, summary_op, summary_writer,
                  summary_batch_step]:
        raise ValueError
      _, total_loss, top_loss, wd_loss, lr, y_pred, y = sess.run(
          [train_op, losses[0], losses[1], losses[2], learning_rate, logits,
           y_var])
    ...
    current_n_correct = (labels_pred[:n_used] == labels[:n_used]).sum()
    ...
    if train_op is not None:
      if batch_iter.batch_idx % summary_batch_step == 0:
        ...
        summary_writer.add_summary(
            summary_str,
            (epoch - 1) * batch_iter.n_batches + batch_iter.batch_idx)
      if batch_iter.batch_idx % console_print_step == 0:
        sys.stdout.write(
            "Minibatch: %d/%d (%.1f%%), accuracy: %.3f, total_loss: %.3f, "
            "top_loss: %.3f, wd_loss: %f, learning rate: %.6f\r" %
            (batch_iter.batch_idx, batch_iter.n_batches,
             100 * batch_iter.batch_idx / float(batch_iter.n_batches),
             current_n_correct / float(n_used), total_loss, top_loss, wd_loss,
             lr))
        sys.stdout.flush()
  if train_op is not None:
    print()
...
def get_train_op(losses, batches_seen, batch_size, train_size, optimizer,
                 base_lr, optimizer_params, lr_decay_step, lr_decay):
...
def train(params):
  ...
  x_train, y_train, x_init, y_init, x_var, y_var = get_dataset_producer(
      batch_size, train_test_ds.train, data_augmenter)
  ...
  train_op, learning_rate = get_train_op(
      list(losses), batches_seen, batch_size, train_size, optimizer, base_lr,
      optimizer_params, lr_decay_step, lr_decay)
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      for epoch in xrange(1, max_epochs + 1):
        ...
        train_accu = run_for_one_epoch(epoch,
                                       sess,
                                       rand_state,
                                       batch_size,
                                       train_test_ds.train,
                                       ...
                                       y_train,
                                       logits_train,
                                       console_print_step=console_print_step,
                                       train_op=train_op,
                                       losses=losses,
                                       learning_rate=learning_rate,
                                       summary_op=summary_op,
                                       summary_writer=summary_writer,
                                       summary_batch_step=summary_batch_step)
        print("Starting test.")
        val_accu = run_for_one_epoch(epoch,
                                     sess,
                                     rand_state,
                                     batch_size,
                                     train_test_ds.test,
        ...
        if epoch % checkpoint_epoch_step == 0:
          ...
          checkpoint_file_path = join(
              checkpoint_path,
              'epochs_completed_%03d_train_accu_%s_val_accu_%s.ckpt' %
              (epoch, train_accu_str, val_accu_str))
          actual = saver.save(sess,
                              checkpoint_file_path,
                              global_step=batches_seen)
          if actual:
            print("\nModel Saved at: %s\n" % actual)
    except KeyboardInterrupt:
      print("\n\nExiting!!\n")
      # TODO: how to save at this point correctly? The commented code below is
      # not working.
      # batches_per_epoch = np.ceil(float(train_size) / batch_size)
      # partial_epochs = ("%.2f" % (batches_seen.eval(sess) /
      #                             batches_per_epoch)).replace('.', '_')
      # checkpoint_file_path = join(checkpoint_path,
      #                             'epochs_partially_completed_%s.ckpt' %
      #                             partial_epochs)
